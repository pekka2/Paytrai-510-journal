<?php 
class ModelExtensionPaymentAktia extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_aktia_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'aktia',
                    'title'      => $this->language->get('text_aktia'),
				    'terms'      => '',
				    'sort_order' => $this->config->get('payment_aktia_sort_order')
      		);
    	}
