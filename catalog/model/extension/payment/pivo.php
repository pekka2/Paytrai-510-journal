<?php 
class ModelExtensionPaymentPivo extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_pivo_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'pivo',
                'title'      => $this->language->get('text_pivo'),
				        'terms'      => '',
				        'sort_order' => $this->config->get('payment_pivo_sort_order')
      		);
    	}
