<?php 
class ModelExtensionPaymentHandelsbanken extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_handelsbanken_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'handelsbanken',
                'title'      =>  $this->language->get('text_handelsbanken'),
				       'terms'      => '',
				       'sort_order' => $this->config->get('payment_handelsbanken_sort_order')
      		);
    	}
