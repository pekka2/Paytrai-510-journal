<?php 
class ModelExtensionPaymentVisa extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_visa_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'visa',
                'title'      => $this->language->get('text_visa'),
				        'terms'      => '',
				        'sort_order' => $this->config->get('payment_visa_sort_order')
      		);
    	}
