<?php 
class ModelExtensionPaymentAlandsbanken extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_alandsbanken_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'alandsbanken',
                'title'      => $this->language->get('text_alandsbanken'),
				        'terms'      => '',
				        'sort_order' => $this->config->get('payment_alandsbanken_sort_order')
      		);
    	}
