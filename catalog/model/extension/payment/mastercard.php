<?php 
class ModelExtensionPaymentMastercard extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_mastercard_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'mastercard',
                 'title'      => $this->language->get('text_mastercard'),
				        'terms'      => '',
