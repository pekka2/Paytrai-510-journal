<?php 
class ModelExtensionPaymentVisaElectron extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_visa_electron_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'visa_electron',
                'title'      => $this->language->get('text_visa_electron'),
				        'terms'      => '',
				        'sort_order' => $this->config->get('payment_visa_electron_sort_order')
      		);
    	}
