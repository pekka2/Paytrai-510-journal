<?php 
class ModelExtensionPaymentSpankki extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_spankki_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'spankki',
                    'title'      => $this->language->get('text_spankki'),
				    'terms'      => '',
				    'sort_order' => $this->config->get('payment_spankki_sort_order')
      		);
    	}
