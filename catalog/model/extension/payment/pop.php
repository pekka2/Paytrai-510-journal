<?php 
class ModelExtensionPaymentPop extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_pop_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'pop',
                    'title'      =>  $this->language->get('text_pop_pankki'),
				    'terms'      => '',
				    'sort_order' => $this->config->get('payment_pop_sort_order')
      		);
    	}
