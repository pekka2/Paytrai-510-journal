<?php 
class ModelExtensionPaymentOsuuspankki extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_osuuspankki_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'osuuspankki',
                'title'      => $this->language->get('text_osuuspankki'),
				        'terms'      => '',
				        'sort_order' => $this->config->get('payment_osuuspankki_sort_order')
      		);
    	}
