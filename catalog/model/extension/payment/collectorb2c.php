<?php 
class ModelExtensionPaymentCollectorB2C extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_collectorb2c_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'collectorb2c',
                'title'      => $this->language->get('text_walley'),
				        'terms'      => '',
				        'sort_order' => $this->config->get('payment_collectorb2c_sort_order')
      		);
    	}
