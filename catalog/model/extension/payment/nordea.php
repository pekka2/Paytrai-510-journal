<?php 
class ModelExtensionPaymentNordea extends Model {
  	public function getMethod($address, $total) {
		$this->load->language('extension/payment/paytrail_510_journal');
		if ($this->config->get('paytrail_510_status') && $this->config->get('payment_nordea_status')) {
		...
		if ($status) {  
      		$method_data = array( 
        		    'code'       => 'nordea',
                'title'      => $this->language->get('text_nordea'),
				        'terms'      => '',
				        'sort_order' => $this->config->get('payment_nordea_sort_order')
      		);
    	}
