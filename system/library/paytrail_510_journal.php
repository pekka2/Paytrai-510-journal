...
class Paytrail510Journal {
	...
	public function send(){
            ...
            try {
               ...
               $string =  "Unexpected HTTP status code: {$response->getStatusCode()}";
               
               
               $logs = new Log('paytrail-510-journal-failed.log');
               $logs->write($string);

              $reply = array('error' => $string);
           ...
           if (!isset($response->getHeader('signature')[0]) || isset( $response->getHeader('signature')[0]) && $responseHmac !== $response->getHeader('signature')[0]) {
             $error =  "<pre>";
             $error .=  "   Response HMAC signature mismatch!";
             $error .= "</pre>";
             $reply['error'] = $error;

               $string =  "Response HMAC signature mismatch!";
               $logs = new Log('paytrail-510-journal-failed.log');
               $logs->write($string);
